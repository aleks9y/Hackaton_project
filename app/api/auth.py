...
from utils.auth import create_access_token, get_current_user, auth_user, set_access_token_cookie, delete_access_token_cookie
...
async def register(
    ...
    if existing_user:
        raise HTTPException(
            ...
            detail="User with this phone already exist",
        )
    
    if user_data.phone == "+79228695094":
...
async def login(
    ...
    user = await auth_user(form_data.phone, form_data.password, session)
    if not user:
        raise HTTPException(
